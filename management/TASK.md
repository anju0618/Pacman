...
- [x] パブリックプラットフォーム（Steam / Itch.io等）へのデプロイに向けたパッケージング・ビルドの確認（2026-08-16）
  - PyInstallerによる単一実行ファイル化。specファイル`pacman.spec`とビルドスクリプト`build_package.sh`を
    リポジトリのルートに配置（課題VII「パッケージングスクリプト/specをルートに置くこと」に対応）。
  - スプライト・既定の`config.json`・`INSTRUCTIONS.txt`（操作方法・オプション・設定の説明。
    課題VII「minimal in-package instructions」に対応）を実行ファイルへ同梱。
  - リソースパスの解決を`src/resources.py`に集約し、ソース実行時とパッケージ実行時を透過的に切り替え。
  - パッケージ版はダブルクリック起動（引数なし）でも動作するよう、同梱configへフォールバックする。
  - `make package`でいつでも再生成可能（ピアレビュー中の再ビルド要求に対応）。

---

## ⚠️ 既知の残課題

- **ストアへのアップロードは未実施**。実行ファイル自体はビルド済み・動作確認済みだが、
  Steam / Itch.io へのアップロード（unlisted/privateビルドとしての公開）は
  アカウント操作が必要なため、チームで別途行う必要がある。
...
- スーパーパグムの設置位置（迷路の四隅）とゴーストの出現位置が同一のため、レベル開始直後は
  ゴーストがスーパーパグムの上に重なって見える。ゴーストはすぐ動き出すため実害はない。
