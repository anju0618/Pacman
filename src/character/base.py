...
class Character:
    ...
    def _collides_with_wall(self, check_x: float, check_y: float, maze_data: list[list[int]]) -> bool:
        ...
        for gy in range(min_y, max_y + 1):
            for gx in range(min_x, max_x + 1):
                if gy < 0 or gy >= len(maze_data) or gx < 0 or gx >= len(maze_data[0]):
                    return True
    ...
    def move_forward(self, maze_data: list[list[int]]) -> bool:
        ...
        if self.next_direction is not None:
            ...
            is_turning_sideways = (
                (self.direction in (Direction.UP, Direction.DOWN) and self.next_direction in (Direction.LEFT, Direction.RIGHT)) or
                (self.direction in (Direction.LEFT, Direction.RIGHT) and self.next_direction in (Direction.UP, Direction.DOWN))
            )

            if is_turning_sideways and (offset_x <= 0.35 and offset_y <= 0.35):
                test_x, test_y = float(grid_x), float(grid_y)
                if self.next_direction == Direction.UP: test_y -= self.speed
                elif self.next_direction == Direction.DOWN: test_y += self.speed
                elif self.next_direction == Direction.LEFT: test_x -= self.speed
                elif self.next_direction == Direction.RIGHT: test_x += self.speed
